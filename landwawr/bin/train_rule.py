...
def run_one_game(module, cls, scenario, num_plays, team_name, team_leader_name, team_code, save_replay = 1):
    ...
    try:
        ...
        for i in range(num_plays):
            ...
            while not game_over:
                flag_game_not_over = ai_red.run()
                if not flag_game_not_over:
                    game_over = True
                flag_game_not_over = ai_blue.run()
                if not flag_game_not_over:
                    game_over = True
                # actions = ai_red.step(observations[ai_red.color], ai_red.color)
                # actions_blue = ai_blue.step(observations[ai_blue.color], ai_red.color)
...
if __name__ == "__main__":
    ...
    scenario_list = [1531,1631,1231]
    time_list = {key: 0 for key in scenario_list}
    num_plays = 1  # 比赛次数
    start = datetime.now().strftime("%m%d%H%M")
    for scenario in scenario_list:
        t1 = time.time()
        run_one_game(module="ai.demo_ai", cls="DemoAI", scenario=scenario,
                     num_plays=num_plays,
                     team_name=args.team_name,
                     team_code=args.code,
                     team_leader_name=args.team_leader,save_replay=1)
        print("done")
        t2 = time.time()
