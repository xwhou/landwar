...
class WeaponUnfolding(State):
    """
    状态类（抽象类）
    """
    need_time = TimeParameter.WeaponUnfoldTime
    ...
    def __init__(self, bop, state_data, action_msg_sets):
        ...
        """
        super(WeaponUnfolding, self).__init__(bop=bop, state_data=state_data, action_msg_sets=action_msg_sets)
        self.type = StateType.WeaponUnFold
        try:
    ...
    def enter(self, action_msg):
        try:
            self.bop.weapon_unfold_time = self.need_time
        except Exception as e:
            print_exception(e)
            raise e
    ...
    def update(self, state_manager):
        try:
            self.bop.weapon_unfold_time -= self.clock.get_tick()
            if self.bop.weapon_unfold_time <= 0:
    ...
    def stop(self, state_manager):
        try:
            self.bop.weapon_unfold_time = 0
            super(WeaponUnfolding, self).stop(state_manager)
        except Exception as e:
            print_exception(e)
            raise e
